  ...
  Future<void> _fetchLocationAndWeather() async {
    try {
      ...
      if (permission == LocationPermission.whileInUse || permission == LocationPermission.always) {
        ...
        await _getWeatherForLatLon(position.latitude, position.longitude);
        return;
      }
    } catch (e) {
      debugPrint("GPS location failed or denied: $e");
    }

    // Fallback to IP-based location and weather if GPS is denied/failed/timed out
    await _fetchWeatherByIP();
  }

  Future<void> _fetchWeatherByIP() async {
    try {
      ...
      if (ipResponse.statusCode == 200) {
        ...
        if (ipData['status'] == 'success') {
          ...
          final String city = ipData['city'] ?? "";
          final String region = ipData['regionName'] ?? ipData['country'] ?? "";
          final String resolvedLoc = region.isNotEmpty ? "$city, $region" : city;
          
          await _getWeatherForLatLon(lat, lon, fallbackLocationName: resolvedLoc);
          return;
        }
      }
    } catch (e) {
      debugPrint("IP Location fetch error: $e");
    }
    _loadFallbackProfileLocation();
  }

  Future<void> _getWeatherForLatLon(double lat, double lon, {String? fallbackLocationName}) async {
    try {
      String resolvedLocation = fallbackLocationName ?? _locationName;

      if (fallbackLocationName == null) {
      ...
      final weatherResponse = await http.get(
        Uri.parse('https://api.open-meteo.com/v1/forecast?latitude=$lat&longitude=$lon&current=temperature_2m,relative_humidity_2m'),
      ).timeout(const Duration(seconds: 5));
      ...
      debugPrint("Error getting weather for lat/lon: $e");
      _loadFallbackProfileLocation();
    }
  }
